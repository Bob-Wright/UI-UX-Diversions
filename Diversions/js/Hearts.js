/*!
 * Hearts.js
 * modified from snowflakeCursor.js
...
//  var possibleChars = ["@", "*", "&empty;", "&cent;", "&sect;", "&plusmn;","&micro;", "&infin;", "&oplus;", "&otimes;", "&Delta;", "&Omega;"];
  ...
  function onTouchMove(e) {
    if( e.touches.length > 0 ) {
      for( var i = 0; i < e.touches.length; i++ ) {
        addParticle( e.touches[i].clientX, e.touches[i].clientY, possibleColors[Math.floor(Math.random()*possibleColors.length)], possibleChars[Math.floor(Math.random()*possibleChars.length)]);
      }
    }
  }
  ...
  function onMouseMove(e) {    
	...
	if ((Math.abs(cursor.x - e.clientX) > (width * .01)) || (Math.abs(cursor.y - e.clientY) > (width * .01))) {
		cursor.y = e.clientY;
		cursor.x = e.clientX;
    addParticle( cursor.x, cursor.y, possibleColors[Math.floor(Math.random()*possibleColors.length)], possibleChars[Math.floor(Math.random()*possibleChars.length)]);
   }
  }
